...
from scrape._eval_report import EvalReport, EvalReportPage, EvalReportQuestion, ScoreSection
...
class RemoteData:
  schema_db: SchemaDB
  evals: bytearray
  unsynced_schema_db: SchemaDB
  unsynced_evals_index: int
  ...
  def count_course(self, course: str) -> int:
    i = 0
    for eval in self.deserialize():
  ...
  async def write(self):
    professors = [{'name': name} for name in self.unsynced_schema_db.professors]
    res = await self.supabase.table('professor').insert(self.unsynced_schema_db.professors).execute()
    courses = [{'name': name} for name in self.unsynced_schema_db.courses]
    res = await self.supabase.table('course').insert(self.unsynced_schema_db.courses).execute()
    sections = [{'name': name} for name in self.unsynced_schema_db.sections]
    res = await self.supabase.table('section').insert(self.unsynced_schema_db.sections).execute()
    
    
    course = [{'name': name} for ]
    self.path.mkdir(parents=True, exist_ok=True)
    schema_path = self.path.joinpath(schema_file_name)
    evals_path = self.path.joinpath(evals_file_name)
    with (
  ...
  def write_json(self, path: str):
    ...
    with open(path_.joinpath(json_file_name), "w") as f:
      json.dump(self.deserialize(), f, indent=2, default=json_default)
# type Data = tuple[SchemaDB, bytearray]

def fetch_schema(path: Path) -> SchemaDB:
  try:
    with open(path.joinpath(schema_file_name)) as f:
...
def fetch_evals(path: Path) -> bytearray:
  try:
    with open(path.joinpath(evals_file_name), "rb") as f:
...
def fetch_data(path: str) -> Data:
  path_ = Path(path)
  path_.mkdir(parents=True, exist_ok=True)
  return Data(fetch_schema(path_), fetch_evals(path_), path=path_)
...
def test():
  ...
  #   f.write(data.evals)
  data2 = fetch_data(path)
  path_ = Path(path)
  test_path = path_.joinpath('test')
  test_path.mkdir(parents=True, exist_ok=True)
  data2.write_json(str(test_path))
