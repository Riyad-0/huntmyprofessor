...
from scrape._eval_report import EvalReportQuestion, ScoreSection
...
def parse_response(res_text: str, cookies: Cookies) -> Output:
  ...
  for grade_name in grade_names:
    grade_name_el = soup.find(name="td", string=grade_name) # type: ignore
    if grade_name_el is None:
      raise HTMLError(res_text, f"Grade name element for '{grade_name}'")
    grade_count_el = grade_name_el.find_next_sibling(name="td") # type: ignore
    if grade_count_el is None:
      raise HTMLError(res_text, f"Grade count element for '{grade_name}'")
    grade_count = grade_count_el.text # type: ignore
    grades.append(int(grade_count)) # type: ignore
